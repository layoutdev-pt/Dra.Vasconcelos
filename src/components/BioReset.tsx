        ...
        <div className="bg-surface rounded-[2rem] p-8 lg:p-12 shadow-xl flex flex-col lg:flex-row items-center gap-12 border border-surface-border backdrop-blur-sm">
          <div className="lg:w-1/2 space-y-6">
            <div className="inline-block px-3 py-1 bg-surface-muted text-secondary text-[10px] font-bold uppercase rounded-full tracking-widest border border-surface-border">
              {latestCourse.type === "programa"
                ? "Programa"
                : latestCourse.type === "presencial"
                  ? "Presencial"
                  : "Curso"}
            </div>
            ...
            <ul className="space-y-4 pt-2">
              {[
                <React.Fragment key="1">
                  ...
                  4 Reuniões Online (Formação Prática via Zoom) e Grupo VIP no
                  Telegram com acompanhamento diário.
                </React.Fragment>,
                <React.Fragment key="2">
                  <span className="font-bold">
                    Conteúdo Programático Exclusivo:
                  </span>{" "}
                  Limpeza total (desparasitação e desintoxicação do fígado,
                  vesícula e intestino), recuperação de energia e criação de
                ...
                <React.Fragment key="3">
                  ...
                  Acesso às gravações das sessões por 3 meses e bónus de 50% de
                  desconto no Teste de Intolerância Alimentar
                </React.Fragment>,
              ].map((item, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-3 text-site-text font-medium leading-relaxed"
                >
