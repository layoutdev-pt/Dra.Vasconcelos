...
export const BlogComments: React.FC<BlogCommentsProps> = ({ postId }) => {
  ...
  return (
    <div className="max-w-[900px] mx-auto mt-20 pt-16 border-t border-gray-100">
      <h3 className="text-2xl font-bold text-primary mb-8 flex items-center gap-3">
        <MessageSquare className="w-6 h-6 text-secondary" /> 
        Comentários <span className="text-gray-400 font-medium text-lg">({comments.length})</span>
      </h3>

      {/* New Comment Input */}
      <div className="mb-12 bg-gray-50 rounded-3xl p-6 border border-gray-100">
        {!user ? (
          <div className="text-center py-6">
            <p className="text-gray-600 mb-4 font-medium">Junta-te à conversa!</p>
          ...
          <div className="text-center py-6 text-red-500 bg-red-50 rounded-xl">
          ...
          <div className="flex flex-col gap-4">
            ...
            <textarea
              className="w-full bg-white border border-gray-200 rounded-2xl p-4 text-gray-700 focus:outline-none focus:ring-2 focus:ring-secondary/30 focus:border-secondary transition-all resize-none"
              rows={3}
            ...
            <div className="flex justify-end">
              <button 
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-primary text-white font-bold text-sm rounded-xl hover:bg-opacity-90 disabled:opacity-50 transition-all shadow-md"
              >
          ...
          {topLevelComments.map(comment => (
            <div key={comment.id} className="flex flex-col gap-3">
              ...
              <div className="bg-white border border-gray-100 p-5 rounded-2xl shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-sm text-primary">{getDisplayName(comment.profiles)}</p>
                      <p className="text-xs text-gray-400">{new Date(comment.created_at).toLocaleString()}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    {user && user.id !== comment.user_id && (
                       <button onClick={() => handleReport(comment.id)} title="Denunciar Abuso" className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Flag className="w-4 h-4" /></button>
                    )}
                    {(isAdmin || user?.id === comment.user_id) && (
                      <button onClick={() => handleDelete(comment.id)} title="Apagar Comentário" className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                    )}
                  </div>
                </div>
                ...
                {user && !isBanned && (
                  <button 
                    ...
                    className="text-xs font-bold text-secondary mt-3 flex items-center gap-1 hover:text-primary transition-colors"
                  >
              ...
              {replyTo === comment.id && user && !isBanned && (
                <div className="ml-10 bg-secondary/5 border border-secondary/20 rounded-xl p-4 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setReplyTo(null)} className="text-gray-400 hover:text-red-500 transition-colors"><X className="w-4 h-4" /></button>
                  </div>
                  <textarea
                    autoFocus
                    className="w-full bg-white border border-gray-200 rounded-xl p-3 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-secondary/30 focus:border-secondary transition-all resize-none"
                    rows={2}
              ...
              {getReplies(comment.id).map(reply => (
                <div key={reply.id} className="ml-10 bg-gray-50 border border-gray-100 p-4 rounded-xl">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      {renderAvatar(reply.profiles, 'sm')}
                      <div>
                        <p className="font-bold text-sm text-primary">{getDisplayName(reply.profiles)}</p>
                        <p className="text-[10px] text-gray-400">{new Date(reply.created_at).toLocaleString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      {user && user.id !== reply.user_id && (
                        <button onClick={() => handleReport(reply.id)} title="Denunciar Abuso" className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Flag className="w-3 h-3" /></button>
                      )}
                      {(isAdmin || user?.id === reply.user_id) && (
                        <button onClick={() => handleDelete(reply.id)} title="Apagar Resposta" className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 className="w-3 h-3" /></button>
                      )}
                    </div>
                  </div>
