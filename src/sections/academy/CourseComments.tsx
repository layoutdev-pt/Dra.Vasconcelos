...
export const CourseComments: React.FC<CourseCommentsProps> = ({ courseId }) => {
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm(
        "Tem a certeza que deseja apagar este comentário permanentemente?",
      )
    )
      return;
    ...
    if (profile?.avatar_url) {
      return (
        <OptimizedImage
          ...
          alt=""
          className={`${dim} rounded-full object-cover border-2 border-secondary/20`}
        />
      );
    }
    return (
      <div
        className={`${dim} rounded-full bg-secondary/10 flex items-center justify-center text-secondary`}
      >
  ...
  return (
    <div className="max-w-[900px] mx-auto mt-20 pt-16 border-t border-surface-border">
      <h3 className="text-2xl font-bold text-site-text mb-8 flex items-center gap-3">
        ...
        <span className="text-site-text-muted font-medium text-lg">
          ({comments.length})
        </span>
      </h3>

      {/* New Comment Input */}
      <div className="mb-12 bg-surface-muted/30 rounded-3xl p-6 border border-surface-border">
        {!user ? (
          <div className="text-center py-6">
            <p className="text-site-text-muted mb-4 font-medium">
              Tens alguma dúvida sobre este curso?
            </p>
            <Link
              to="/entrar"
              className="inline-flex px-6 py-2.5 bg-secondary text-white font-bold rounded-xl hover:bg-secondary/90 transition-colors shadow-lg shadow-secondary/20"
            >
          ...
          <div className="text-center py-6 text-red-500 bg-red-500/10 rounded-xl border border-red-500/20">
            ...
            <p className="font-bold">
              A tua conta foi bloqueada devido a Múltiplas Denúncias.
            </p>
            <p className="text-sm mt-1">
              Perdeste o acesso de participação no portal.
            </p>
          </div>
        ) : (
            ...
            <div className="flex items-center gap-3">
              {user.user_metadata?.avatar_url ? (
                <OptimizedImage
                  ...
                  alt=""
                  className="w-8 h-8 rounded-full object-cover border-2 border-secondary/20"
                />
              ) : (
              ...
              <p className="text-sm font-semibold text-site-text-muted">
                {user.email}
              </p>
            </div>
            <textarea
              ...
              rows={3}
              placeholder="Escreve aqui a tua dúvida ou comentário sobre o curso..."
              value={newComment}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-site-text-muted">
          ...
          <p>Ainda não há comentários. Sê o primeiro a perguntar!</p>
        </div>
      ) : (
        <div className="space-y-8">
          {topLevelComments.map((comment) => (
            <div key={comment.id} className="flex flex-col gap-3">
              {/* Parent Comment */}
              <div className="bg-surface border border-surface-border p-5 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    {renderAvatar(comment.profiles)}
                    <div>
                      <p className="font-bold text-sm text-site-text">
                  ...
                  <div className="flex items-center gap-1">
                    {user && user.id !== comment.user_id && (
                      <button
                        ...
                        className="p-1.5 text-site-text-muted hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors"
                      >
                ...
                <p className="text-site-text-muted text-sm leading-relaxed whitespace-pre-wrap">
                  {comment.content}
                </p>
                ...
                <div className="ml-10 bg-secondary/5 border border-secondary/20 rounded-xl p-4 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-secondary font-bold flex items-center gap-1">
                      <CornerDownRight className="w-3 h-3" /> A responder a{" "}
                      {getDisplayName(comment.profiles)}
                    </p>
                    <button
                      onClick={() => setReplyTo(null)}
                      className="text-site-text-muted hover:text-red-500 transition-colors"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
              ...
              {getReplies(comment.id).map((reply) => (
                <div
                  key={reply.id}
                  className="ml-10 bg-surface-muted/50 border border-surface-border p-4 rounded-xl"
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-site-text-muted">
                  ...
                  <p className="text-site-text-muted text-sm leading-relaxed">
                    <span className="text-secondary font-medium text-xs mr-2">
                      @{getDisplayName(comment.profiles)}
                    </span>
                    {reply.content}
                  </p>
