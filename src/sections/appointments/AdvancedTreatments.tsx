    ...
    <section className="py-24 bg-site-bg relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="flex justify-center mb-10 w-full overflow-x-auto pb-4 hide-scrollbar">
          <div className="inline-flex bg-surface/70 backdrop-blur-md p-1.5 rounded-full border border-surface-border shadow-[0_4px_20px_-5px_rgba(124,176,176,0.15)] flex-nowrap min-w-max">
            {TABS.map(tab => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-2 px-6 py-3 rounded-full text-sm font-semibold transition-all duration-300 ${
                    isActive ? 'text-white' : 'text-site-text-muted hover:text-secondary hover:bg-secondary/5'
                  }`}
                >
                  {isActive && (
                    <motion.div
                  ...
                  <div className="relative z-10 flex items-center gap-2">
                    <Icon className={`w-4 h-4 ${isActive ? 'text-white' : 'text-site-text-muted/60'}`} />
                    {tab.label}
                  </div>
                </button>
              );
              ...
              >
                <div className="flex flex-col h-full justify-center">
                  ...
                  <div className="flex items-center gap-3 py-3 px-5 bg-secondary/5 rounded-2xl border border-secondary/10 w-fit">
                    ...
                    <p className="text-sm text-site-text font-semibold">100% isento de radiação nociva</p>
                  </div>
                </div>

                <div className="bg-surface-muted rounded-4xl p-8 border border-surface-border h-full flex flex-col justify-center relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-secondary/10 blur-2xl rounded-full" />
                  <h4 className="font-bold text-site-text text-lg mb-6 relative z-10 flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-2.5 relative z-10">
                    {THERMOGRAPHY_SYSTEMS.map(tag => (
                      <span
                        key={tag}
                        className="px-4 py-2 bg-surface border border-surface-border rounded-xl text-sm text-site-text font-medium shadow-xs"
                      >
                        {tag}
                      </span>
                    ))}
                  </div>
                  <p className="text-xs text-site-text-muted/60 mt-8 relative z-10 text-center uppercase tracking-widest font-semibold">
              ...
              >
                ...
                <div className="lg:col-span-3 flex flex-col gap-4">
                  {COMPLEMENTARY_EXAMS.map((exam, i) => {
                    ...
                    return (
                      <div key={i} className="bg-surface-muted p-5 rounded-2xl border border-surface-border hover:border-secondary/20 transition-colors">
                        <div className="flex gap-4">
                          <div className="w-12 h-12 bg-surface rounded-xl shadow-sm flex items-center justify-center shrink-0 border border-surface-border">
                            <Icon className="w-5 h-5 text-secondary" />
                          </div>
                          <div>
                            <h4 className="font-bold text-site-text text-base mb-1.5">{exam.title}</h4>
                            <p className="text-site-text-muted font-light text-sm leading-relaxed mb-3">{exam.detail}</p>
                            <div className="flex flex-wrap gap-2">
                              {exam.tags.map(tag => (
                                <span key={tag} className="px-2.5 py-1 bg-surface border border-surface-border rounded-lg text-xs text-secondary font-medium">
                                  {tag}
                                </span>
                              ))}
                            </div>
