...
export const JourneyTimeline: React.FC = () => {
  ...
  const milestones = [
    ...
    },
    {
      ...
      description: "Neste ano, optei por abandonar a prática de farmácia de oficina para me dedicar ao estudo e à formação na Medicina Natural. Procurei incessantemente terapêuticas biológicas que respeitassem a genética individual de cada pessoa, e levando sempre em conta a vertente física, mental e espiritual."
    },
    {
      year: "Hoje",
      title: "O Ponto Viragem e Propósito",
      description: "O resultado que vejo diariamente nos pacientes é a minha grande motivação. Sinto-me extremamente grata pela vida por me ter dado a convicção de investir na área da Medicina Biológica Integrativa, permitindo-me desempenhar um papel determinante na modelação das doenças crónicas."
    }
  ];

  return (
    <section ref={sectionRef} className="relative py-32 bg-primary text-white overflow-hidden transition-colors duration-500">
            ...
            {milestones.map((item, idx) => {
              ...
              <div key={idx} className={`relative flex flex-col md:flex-row items-start ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className="absolute left-0 md:left-1/2 top-0 flex items-center justify-center transform md:-translate-x-1/2">
                  <div className={`w-[56px] h-[56px] rounded-full bg-primary border-4 flex items-center justify-center z-10 transition-colors duration-500 ${isActive ? 'border-secondary shadow-[0_0_20px_rgba(124,176,176,0.4)]' : 'border-white/10'}`}>
                ...
                <div className={`hidden md:block w-1/2 pt-4 ${idx % 2 === 0 ? 'text-left pl-16' : 'text-right pr-16'}`}>
                  <span className={`text-5xl font-black transition-all duration-500 ${isActive ? 'text-secondary opacity-100' : 'text-white/5'}`}>{item.year}</span>
                </div>

                {/* Cartão de Conteúdo */}
                <div className={`w-full pl-20 md:pl-0 md:w-1/2 ${idx % 2 === 0 ? 'md:pr-16' : 'md:pl-16'}`}>
                  <div className={`border p-8 rounded-2xl backdrop-blur-sm transition-all duration-500 ${isActive ? 'bg-white/10 border-secondary/30 scale-100 shadow-xl' : 'bg-white/5 border-white/10 scale-95 opacity-50'}`}>
                    <div className={`md:hidden font-black text-3xl mb-4 transition-colors duration-500 ${isActive ? 'text-secondary' : 'text-white/10'}`}>{item.year}</div>
